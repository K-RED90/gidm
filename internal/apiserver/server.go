...
// New builds a Server over mgr. A nil logger defaults to slog.Default(). Each
// non-positive timeout/limit in cfg is floored to its package default so a
// hand-built config.Daemon{} (as in some tests) never yields a zero deadline.
